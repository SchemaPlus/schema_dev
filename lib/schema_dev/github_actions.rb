  ...
  module GithubActions
    ...
    DB_SETUP           = [
      {
        ...
        if:   %q{(matrix.db == 'postgresql' || matrix.db == 'mysql2')},
        run:  <<~BASH
          COUNT=0
          ATTEMPTS=20
    ...
    DB_TEARDOWN        = [
      {
        ...
        if:   %q{always() && (matrix.db == 'postgresql' || matrix.db == 'mysql2')},
    ...
    def build_matrices(config)
      ...
      if config.db.include?('postgresql')
        ...
        config.matrix(db: 'postgresql', with_dbversion: true).map { |entry|
    ...
    def update(config)
      ...
      oldworkflow = YAML.load(filepath.read) rescue nil
